...
class SlTaskScheduleController extends Controller
{
	...
	{	
		...
		echo __LINE__."  $m KB\r\n";
		$statArr = array(SlTaskScheduleCrontabConsole::TASK_STATUS_EXECUTING, SlTaskScheduleCrontabConsole::TASK_STATUS_UNSTARTED);

		Yii::$app->db->createCommand(
				"CREATE TABLE IF NOT EXISTS `temp_tb_sl_stat` (" . 
				  "`s` tinyint(1) unsigned NOT NULL DEFAULT '0'," .
				  "PRIMARY KEY (`s`)" .
				") ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='临时表stat';"
			)->execute();

		$insertTempValues = '';
		foreach ($statArr as $s) 
		{
			$insertTempValues .= '(' . $s . '),';
		}
		Yii::$app->db->createCommand('TRUNCATE `temp_tb_sl_stat`;')->execute();
		Yii::$app->db->createCommand('INSERT INTO `temp_tb_sl_stat` (s) VALUES' . substr($insertTempValues, 0, -1) . ' ON DUPLICATE KEY UPDATE s = values(s);')->execute();
		unset($statArr, $insertTempValues);
		/*$crontabIdArr = SlTaskScheduleCrontabConsole::find()
			->alias('cron')
			->joinWith('schedule')
			->leftJoin('temp_tb_sl_stat s', 'cron.task_status = s.s')
			->select('sche.alert_params, cron.id, cron.sche_id, cron.name, cron.act_time, cron.start_time')
			->asArray()
			->indexBy('id')
			->all();*/
		$sth = Yii::$app->db->masterPdo->prepare('SELECT sche.alert_params, cron.id, cron.sche_id, cron.name, cron.act_time, cron.start_time FROM sl_task_schedule_crontab '.
									' cron LEFT JOIN sl_task_schedule sche ON cron.sche_id = sche.id LEFT JOIN temp_tb_sl_stat s ON cron.task_status = s.s');
		$m = memory_get_usage() / 1024;
		echo __LINE__."  $m KB\r\n";
		$sth->execute();
		$m = memory_get_usage() / 1024;
		...
		/*$cronUnpageArr = SlTaskItemConsole::find()
			...
			->all();*/
		$cronUnpageArr = [];
		$idArr = array_keys( $crontabIdArr );
		$m = memory_get_usage() / 1024;
		echo __LINE__."  $m KB\r\n";
		Yii::$app->db->createCommand(
				"CREATE TABLE IF NOT EXISTS `temp_tb_sl_cron_id` (" . 
				  "`id` tinyint(1) unsigned NOT NULL DEFAULT '0'," .
				  "PRIMARY KEY (`id`)" .
				") ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='临时表cron_id';"
			)->execute();
		...
		Yii::$app->db->createCommand('TRUNCATE `temp_tb_sl_cron_id`;')->execute();
		Yii::$app->db->createCommand('INSERT INTO `temp_tb_sl_cron_id` (id) VALUES' . substr($insertTempValues, 0, -1) . ' ON DUPLICATE KEY UPDATE id = values(id);')->execute();
		unset($idArr, $insertTempValues);
		$m = memory_get_usage() / 1024;
		echo __LINE__."  $m KB\r\n";
		/*$itemArr = SlTaskItemConsole::find()
			->alias('i')
			->select('i.id, i.cron_id, i.paging, i.act_time')
			->leftJoin('temp_tb_sl_cron_id c', 'i.id = c.id')
			->indexBy('id')
			->asArray()
			->all();*/
		$m = memory_get_usage() / 1024;
		echo __LINE__."  $m KB\r\n";	
		$sth = Yii::$app->db->masterPdo->prepare('SELECT i.id, i.cron_id, i.paging, i.act_time FROM sl_task_item '.
									' i LEFT JOIN temp_tb_sl_cron_id c ON i.cron_id = c.id');
		$sth->execute();
		$itemArr = array();
		while( $row = $sth->fetch(\PDO::FETCH_ASSOC) )
		{
		...
		foreach ($itemArr as $item) 
		{
			...
			else
			{
				...
				// var_dump( $item['cron_id'], $crontabIdArr );exit;
				//crontab record does not exist
				if(!isset($crontabIdArr[$item['cron_id']]))
				{
					continue;
				}
		...
		echo __LINE__."  $m KB\r\n";
		Yii::$app->db->createCommand(
				"CREATE TABLE IF NOT EXISTS `temp_tb_sl_item_id` (" . 
				  "`id` tinyint(1) unsigned NOT NULL DEFAULT '0'," .
				  "PRIMARY KEY (`id`)" .
				") ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='临时表item_id';"
			)->execute();
		...
		Yii::$app->db->createCommand('TRUNCATE `temp_tb_sl_item_id`;')->execute();
		Yii::$app->db->createCommand('INSERT INTO `temp_tb_sl_item_id` (id) VALUES' . substr($insertTempValues, 0, -1) . ' ON DUPLICATE KEY UPDATE id = values(id);')->execute();
		$m = memory_get_usage() / 1024;
		echo __LINE__."  $m KB\r\n";
		unset($idArr, $insertTempValues);
		$m = memory_get_usage() / 1024;
		...
		/*$itemPageArr = SlWsDataTaskPageConsole::find()
			->alias('p')
			->select('p.item_id')
			->leftJoin('temp_tb_sl_item_id i', 'p.item_id = i.id')
			->indexBy('item_id')
			->asArray()
			->all();*/
		$sth = Yii::$app->db->masterPdo->prepare('SELECT p.item_id FROM sl_ws_data_task_page '.
									'p LEFT JOIN temp_tb_sl_item_id i ON p.item_id = i.id');
		$sth->execute();
		$m = memory_get_usage() / 1024;
		echo __LINE__."  $m KB\r\n";
		$itemPageArr = array();
		...
		unset($cronIds, $cronUnpageArr, $cronUnpageIdArr, $itemUnpageIdArr, $itemPageIdArr, $cronPageIds);

		Yii::$app->db->createCommand(
				"CREATE TABLE IF NOT EXISTS `temp_tb_sl_page_cron_id` (" . 
				  "`id` tinyint(1) unsigned NOT NULL DEFAULT '0'," .
				  "PRIMARY KEY (`id`)" .
				") ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='临时表page_cron_id';"
			)->execute();
		...
		Yii::$app->db->createCommand('TRUNCATE `temp_tb_sl_page_cron_id`;')->execute();
		Yii::$app->db->createCommand('INSERT INTO `temp_tb_sl_page_cron_id` (id) VALUES' . substr($insertTempValues, 0, -1) . ' ON DUPLICATE KEY UPDATE id = values(id);')->execute();
		unset($insertTempValues, $idArr);
		/*$taskPageArr = SlWsDataTaskPageConsole::find()
			->alias('p')
			->select('p.id, p.task_id, p.state')
			->leftJoin('temp_tb_sl_page_cron_id c', 'p.task_id=c.id')//此处 task_id 对应 task_schedule_crontab表的id 而不是 task_item的id
			->asArray()
			->all();*/

		//开始计算已分页的crontab的进度和状态
		$sth = Yii::$app->db->masterPdo->prepare('SELECT p.id, p.task_id, p.state FROM sl_ws_data_task_page '.
									'p LEFT JOIN temp_tb_sl_page_cron_id c ON p.task_id=c.id');
		$sth->execute();
		// $taskPageArr = array();
		$cronPageProgress = [];
		while( $pVal = $sth->fetch(\PDO::FETCH_ASSOC) )
		{
			// $taskPageArr[]	= $row;
		...
		if(!empty($taskCrontabValues))
		{
			...
			unset($scheCrontabSql, $taskCrontabValues, $scheCrontabSql1);
		}
		/***更新cron END***/
		...
		/*$pageArr = SlWsDataTaskPageConsole::find()
			->alias('p')
			->select('p.item_id, p.task_id, p.state')
			->leftJoin('temp_tb_sl_cron_id c', 'p.task_id = c.id')
			->asArray()
			->all();*/

		$sth = Yii::$app->db->masterPdo->prepare('SELECT p.item_id, p.task_id, p.state FROM sl_ws_data_task_page '.
									'p LEFT JOIN temp_tb_sl_cron_id c ON p.task_id = c.id');
		$sth->execute();
		// $pageArr = array();
		$pageIdArr = array();
		while( $pv = $sth->fetch(\PDO::FETCH_ASSOC) )
		{
			// $pageArr[]	= $row;
		...
		unset($cronItemActTimeArr, $itemCompleteTimeArr, $itemState, $itemProgressArr, $itemStateArr, $itemArr, $pageIdArr, $itemIdArr);
		...
		if(!empty($updateItemValues))
		{
			...
			unset($updateItemSql, $updateItemValues, $updateItemSql1);
		}
		/***更新task_item END***/
